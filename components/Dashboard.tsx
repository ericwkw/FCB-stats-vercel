...
import { TrendingUp, Award, Activity, Shield, Zap, Info } from 'lucide-react';
...
const Dashboard: React.FC = () => {
  ...
  const sortedByAssists = [...stats].sort((a, b) => b.assists - a.assists).slice(0, 5);
  const sortedByCleanSheets = [...stats].filter(p => p.cleanSheets > 0).sort((a, b) => b.cleanSheets - a.cleanSheets).slice(0, 5);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-gradient-to-br from-pitch-600 to-pitch-800 rounded-2xl p-6 text-white shadow-lg relative overflow-visible">
           <div className="absolute right-0 top-0 opacity-10 transform translate-x-4 -translate-y-4 pointer-events-none">
              <Award size={100} />
           </div>
           <div className="flex items-center gap-2 mb-1 relative z-10">
               <h3 className="text-pitch-100 text-sm font-medium">Top Rated Player</h3>
               <div className="relative group cursor-help">
                   <Info size={14} className="text-pitch-200" />
                   <div className="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 p-2 bg-black/90 text-xs text-white rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50 text-center">
                       Weighted Rating = (Position Points) × (Match Difficulty) × (Stadium Size) × (Age Group)
                   </div>
               </div>
           </div>
           {sortedByRating[0] ? (
               ...
               <div className="text-xl relative z-10">No Data</div>
           )}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
           <div className="flex items-center gap-3 mb-2">
             ...
             <span className="text-gray-500 dark:text-gray-400 font-medium">Top Assists</span>
           </div>
           {sortedByAssists[0] ? (
               <div>
                  <div className="text-2xl font-bold dark:text-white">{sortedByAssists[0].name}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">{sortedByAssists[0].assists} Assists</div>
               </div>
           ) : <div className="dark:text-white">--</div>}
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
           <div className="flex items-center gap-3 mb-2">
             ...
             <span className="text-gray-500 dark:text-gray-400 font-medium">Top Clean Sheets</span>
           </div>
           {sortedByCleanSheets[0] ? (
               <div>
                  <div className="text-2xl font-bold dark:text-white">{sortedByCleanSheets[0].name}</div>
                  <div className="text-sm text-gray-500 dark:text-gray-400">{sortedByCleanSheets[0].cleanSheets} Clean Sheets</div>
               </div>
           ) : <div className="dark:text-white">--</div>}
        </div>
      </div>
         ...
         <div className="space-y-6">
             ...
             <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <h3 className="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                    <TrendingUp size={18} className="text-blue-500"/> Top Scorers
                </h3>
                <div className="space-y-3">
                   {sortedByGoals.slice(0, 3).map((p, idx) => (
                      <div key={p.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                         <div className="flex items-center gap-3">
                            <div className={`w-5 text-center font-bold text-sm ${idx < 3 ? 'text-pitch-500' : 'text-gray-400'}`}>{idx + 1}</div>
             ...
             <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
                <h3 className="font-bold text-lg mb-4 dark:text-white flex items-center gap-2">
                    <Zap size={18} className="text-yellow-500"/> Top Assists
                </h3>
                <div className="space-y-3">
                   {sortedByAssists.slice(0, 3).map((p, idx) => (
                      <div key={p.id} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                         <div className="flex items-center gap-3">
                            <div className={`w-5 text-center font-bold text-sm ${idx < 3 ? 'text-pitch-500' : 'text-gray-400'}`}>{idx + 1}</div>
                            <img src={p.avatarUrl} className="w-8 h-8 rounded-full" alt={p.name} />
                         ...
                         <span className="font-bold text-gray-900 dark:text-white">{p.assists}</span>
                      </div>
                   ))}
                </div>
             </div>
